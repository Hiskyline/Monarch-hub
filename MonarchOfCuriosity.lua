...
-- =====================================================
-- Monarch of Curiosity (Rayfield Hub) - Paste & Play
-- Credits: Monarch of Curiosity
-- No key, auto settings save, manual feature toggles
-- =====================================================

repeat task.wait() until game and game:IsLoaded() and game:GetService("Players").LocalPlayer
...
getgenv().MoC = getgenv().MoC or {}

-- ========= Settings (auto-saved) =========
local SETTINGS_FILE = "moc_settings.json"
local Settings = {
...
-- ========= Auto Join Marines (before UI) =========
local function JoinTeamMarines()
...
if Settings.AutoJoinMarines then JoinTeamMarines() end
...
local Window = Rayfield:CreateWindow({
    ...
    LoadingSubtitle = "No-key • Auto-save • Multi-tool",
    ConfigurationSaving = {
        Enabled = false,
        FolderName = "Monarch",
        FileName = "Rayfield"
...
local TabLevel = Window:CreateTab("Level Farm", 4483362458)
local TabChest = Window:CreateTab("Chest Farm", 4483362458)
local TabFruit = Window:CreateTab("Fruit Finder", 4483362458)
local TabMisc  = Window:CreateTab("Misc / Codes", 4483362458)
local TabSettings = Window:CreateTab("Settings", 4483362458)

-- ========= Core writer/loader =========
local CORE_FILE = "moc_core.lua"
local function ensureCore()
    if not isfile or not writefile then return end
    if not isfile(CORE_FILE) then
        writefile(CORE_FILE, [[--[[
Monarch of Curiosity — Auto Level Core
This file is generated by the Monarch Hub loader. Credits: Monarch of Curiosity.
]]
...
-- ========= Utility =========
local function msg(t) Rayfield:Notify({Title="Monarch", Content=t, Duration = 5}) end

-- ========= GLOBAL BRIDGE for core =========
getgenv()._G = getgenv()._G or {}
_G.Settings = _G.Settings or {}
_G.Settings.Main = _G.Settings.Main or {}

-- UI - Home
TabHome:CreateParagraph({Title="Monarch of Curiosity", Content="Welcome! Toggle what you need in each tab.\nAuto-save is on. Credits: Monarch of Curiosity."})

TabSettings:CreateToggle({
    Name = "Auto Join Marines (on start)",
    CurrentValue = Settings.AutoJoinMarines,
    Callback = function(v)
        Settings.AutoJoinMarines = v
        saveSettings()
        if v then JoinTeamMarines() end
    end
})
...
TabSettings:CreateSlider({
    Name = "Server Hop Delay (sec)",
    Range = {3, 15},
    Increment = 1,
    CurrentValue = Settings.HopDelaySec,
    Callback = function(val)
        Settings.HopDelaySec = val
        saveSettings()
    end
})
...
local function runCore()
    if not isfile or not readfile then
        msg("Your executor must support readfile/writefile.")
        return
    end
    if coreRunning then return end
    coreRunning = true
    local ok, err = pcall(function()
        loadstring(readfile(CORE_FILE))()
    end)
    if not ok then
        coreRunning = false
        msg("Core failed: ".. tostring(err))
    else
        msg("Level Farm core loaded.")
    end
end

TabLevel:CreateToggle({
    Name = "Enable Level Farm (Core)",
    CurrentValue = Settings.LevelFarm,
    Callback = function(v)
        ...
        if v then runCore() end
    end
})

TabLevel:CreateToggle({
    Name = "Fast Auto Farm (Core)",
    CurrentValue = false,
    Callback = function(v)
        _G.Settings.Main["Fast Auto Farm Level"] = v
    end
})

-- ========= Fruit Finder (trimmed, webhook optional) =========
local FruitStatus = TabFruit:CreateParagraph({Title="Fruit Status", Content="Idle"})
local fruitRunning = false
local function updateFruitStatus(t) FruitStatus:Set({Title="Fruit Status", Content=t}) end

local function sendWebhook(txt)
    ...
    pcall(function()
        syn.request({
            ...
            Body = HttpService:JSONEncode({
                content = "**Monarch Fruit Finder**",
                embeds = { { title = "Found Fruit", description = txt, color = 65280, timestamp = os.date("!%Y-%m-%dT%H:%M:%SZ") } }
            })
        })
    end)
end

local function fruitCollect(item)
    ...
    if item:IsA("Tool") then
        ...
        if h then
            hrp.CFrame = h.CFrame + Vector3.new(0,3,0)
            task.wait(0.2)
            if not item:IsDescendantOf(workspace) then return true end
        end
    elseif item:IsA("Model") then
        local base
        for _,d in ipairs(item:GetDescendants()) do if d:IsA("BasePart") then base=d break end end
        if base then
            local t0=tick()
            repeat
...
TabFruit:CreateToggle({
    ...
    Callback = function(v)
        Settings.FruitFinder = v
        saveSettings()
        if fruitRunning or not v then
            fruitRunning = v
            return
        end
        fruitRunning = true
        task.spawn(function()
            while fruitRunning do
                local found=false
                updateFruitStatus("Scanning...")
                for _,v2 in ipairs(workspace:GetChildren()) do
                    if v2:IsA("Tool") and v2.Name:lower():find("fruit") then
                        found=true
                        updateFruitStatus("Found: "..v2.Name.." (collecting)")
                        if fruitCollect(v2) then sendWebhook(v2.Name) end
                        break
                    end
                end
                if not found then
                    for _,v2 in ipairs(workspace:GetChildren()) do
                        if v2:IsA("Model") and v2.Name:lower():find("fruit") then
                            found=true
                            updateFruitStatus("Found model fruit (collecting)")
                            if fruitCollect(v2) then sendWebhook("Model Fruit") end
                            break
                        end
                    end
                end
                if not found then updateFruitStatus("None found") end
                task.wait(2)
            end
            updateFruitStatus("Stopped")
        end)
    end
})

-- ========= Chest Farm (Sea-aware, gentle TP, server hop) =========
local ChestStatus = TabChest:CreateParagraph({Title="Chest Status", Content="Idle"})
local chestRunning = false
local visited = {}
local function updateChestStatus(t) ChestStatus:Set({Title="Chest Status", Content=t}) end
...
local SeaLocations = {
    [1] = {
        CFrame.new(1109.97, 16.31, 1432.43), -- Starter Pirate
        CFrame.new(-2753.58, 24.53, 2053.15), -- Starter Marine
        CFrame.new(-1508.9, 11.89, -203.15), -- Jungle
        CFrame.new(-1163.05, 4.79, 3819.38), -- Pirate Village
    },
    [2] = {
        CFrame.new(-16.19, 39.33, 2703.02), -- KOR
        CFrame.new(-381.86,73.08,299.93),   -- Cafe
        CFrame.new(-2298.46,73.04,-2780.78) -- Green Zone
    },
    [3] = {
        CFrame.new(-341.59, 20.68, 5539.96), -- Port Town
        CFrame.new(2183.73, 21.79, -6690.29), -- Great Tree
        CFrame.new(-10900.64, 331.83, -8680.53) -- Floating Turtle
    }
}

local function gentleTP(cf)
    local char = plr.Character
    if not char or not char:FindFirstChild("HumanoidRootPart") then return end
    local hrp = char.HumanoidRootPart
    -- small tween to reduce kick chance
    ...
    for i=1,steps do
        local alpha = i/steps
        local pos = hrp.Position:Lerp(cf.Position, alpha)
...
local function getAllChests()
    ...
    if cm then
        for _,m in ipairs(cm:GetChildren()) do
            if m:IsA("Model") and m.PrimaryPart then
                table.insert(arr, m.PrimaryPart)
            end
        end
    end
    -- fallback search
...
local totalChests = 0
local LOG_FILE = "moc_chestlog.json"
local function loadChestLog()
    if isfile and isfile(LOG_FILE) then
        local ok, data = pcall(function() return HttpService:JSONDecode(readfile(LOG_FILE)) end)
        if ok and typeof(data)=="table" then
            totalChests = tonumber(data.total or 0) or 0
            visited = data.visited or {}
        end
    end
end
local function saveChestLog()
    if writefile then
        writefile(LOG_FILE, HttpService:JSONEncode({total=totalChests, visited=visited}))
    end
end
loadChestLog()

local CounterLabel = TabChest:CreateParagraph({Title="Total Chests", Content=tostring(totalChests)})
local function updateCounter() CounterLabel:Set({Title="Total Chests", Content=tostring(totalChests)}) end

local function hopServer()
    updateChestStatus("Server hopping...")
    local ok, res = pcall(function()
        return HttpService:JSONDecode(game:HttpGet("https://games.roblox.com/v1/games/"..game.PlaceId.."/servers/Public?sortOrder=Asc&limit=100"))
    end)
    if ok and res and res.data then
        for _,s in ipairs(res.data) do
            if s.id ~= game.JobId and not table.find(visited, s.id) then
                table.insert(visited, s.id)
                saveChestLog()
                task.wait(Settings.HopDelaySec)
...
TabChest:CreateToggle({
    Name = "Enable Chest Farm",
    CurrentValue = Settings.ChestFarm,
    Callback = function(v)
        Settings.ChestFarm = v
        saveSettings()
        if chestRunning or not v then
            chestRunning = v
            return
        end
        chestRunning = true
        task.spawn(function()
            while chestRunning do
                ...
                if #chests == 0 then
                    updateChestStatus("No chests here → waiting 5s")
                    task.wait(5)
                    hopServer()
                else
                    updateChestStatus("Collecting ".. tostring(#chests) .." chests...")
                    -- teleport via island hops if far
                    local sea = getSea()
                    for _,cf in ipairs(SeaLocations[sea] or {}) do
                        gentleTP(cf); task.wait(0.2)
                    end
                    for i,c in ipairs(chests) do
                        if not chestRunning then break end
                        if c and c.Parent then
                            gentleTP(c.CFrame + Vector3.new(0,3,0))
                            -- touch
                            local char = plr.Character
                            if char and char:FindFirstChild("HumanoidRootPart") then
                                pcall(function()
                                    firetouchinterest(char.HumanoidRootPart, c, 0)
                                    task.wait(0.2)
                                    firetouchinterest(char.HumanoidRootPart, c, 1)
                                end)
                            end
                            totalChests += 1
                            saveChestLog()
                            updateCounter()
                            task.wait(math.random(2,4)) -- 2-4s gap per chest
                        end
                    end
                end
                task.wait(0.5)
            end
            updateChestStatus("Stopped")
        end)
    end
})
...
-- ========= Codes (simple redeem) =========
local Codes = {
    "SUB2GAMERROBOT_EXP1","SUB2GAMERROBOT_RESET1","EXP_5B", "kittgaming", "TantaiGaming",
    "Starcodeheo","Magicbus","Enyu_is_Pro","JCWK","Bluxxy"
}
TabMisc:CreateButton({
    ...
    Callback = function()
        for _,c in ipairs(Codes) do
            pcall(function()
                ReplicatedStorage.Remotes.CommF_:InvokeServer("RedeemCode", c)
            end)
            task.wait(0.1)
        end
        msg("Codes redeemed.")
...
-- ========= Fast Attack (lightweight) =========
TabMisc:CreateToggle({
    Name = "Fast Attack (basic)",
    CurrentValue = Settings.FastAttack,
    Callback = function(v)
        Settings.FastAttack = v
        saveSettings()
        if v then
            if not MoC.fastAttackConn then
                MoC.fastAttackConn = RunService.RenderStepped:Connect(function()
            ...
            if MoC.fastAttackConn then MoC.fastAttackConn:Disconnect(); MoC.fastAttackConn=nil end
        end
    end
})

msg("Monarch Hub loaded. Configure tabs as you like.")
