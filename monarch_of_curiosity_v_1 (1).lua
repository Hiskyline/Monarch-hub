...
--========================================================--
-- MONARCH PATCH v1.1 — stability + nil-safety (drop-in)
--========================================================--
local _monarch_ok, _ = pcall(function()
    -- Service cache & globals (idempotent)
    local S = rawget(_G, "__MON_SVC")
    if not S then
        S = setmetatable({}, {__index=function(t,k) local s=game:GetService(k); rawset(t,k,s); return s end})
        rawset(_G, "__MON_SVC", S)
    end
    local Players, RunService, TweenService = S.Players, S.RunService, S.TweenService
    local ReplicatedStorage, HttpService, TeleportService = S.ReplicatedStorage, S.HttpService, S.TeleportService
    local VirtualUser = S.VirtualUser

    local LP = Players.LocalPlayer
    local G = (getgenv and getgenv()) or _G; _G = G
    G.AutoFarm = (G.AutoFarm == true)
    G.ClickAttack = (G.ClickAttack ~= false)

    -- Anti‑AFK (safe even if exploit lacks getconnections)
    pcall(function()
        if getconnections then for _,v in pairs(getconnections(LP.Idled)) do v:Disable() end end
    end)
    if not rawget(_G, "__MON_AFK") then
        LP.Idled:Connect(function()
            pcall(function()
                VirtualUser:CaptureController()
                VirtualUser:ClickButton2(Vector2.new())
    ...
    -- Helpers exposed (used by rest of script if names collide)
    local function waitForHRP()
        local ch = LP.Character or LP.CharacterAdded:Wait()
        return ch:WaitForChild("HumanoidRootPart", 5)
    end
    _G.MonWaitForHRP = _G.MonWaitForHRP or waitForHRP

    local function tweenTo(cf)
        local hrp = waitForHRP(); if not hrp then return end
        local dist = (cf.Position - hrp.Position).Magnitude
        local speed = (dist < 10 and 1000) or (dist < 170 and 350) or (dist < 1000 and 350) or 300
        if dist < 170 then hrp.CFrame = cf end
        TweenService:Create(hrp, TweenInfo.new(math.max(0.05, dist/(speed))), {CFrame = cf}):Play()
    end
    _G.MonTweenTo = _G.MonTweenTo or tweenTo

    _G.MonEquipWeapon = _G.MonEquipWeapon or function(name)
        if not name or name=="" then return end
        local ch = LP.Character or LP.CharacterAdded:Wait()
        local hum = ch:FindFirstChildOfClass("Humanoid")
        local bp = LP.Backpack
        if not hum or not bp then return end
        local t = ch:FindFirstChild(name) or bp:FindFirstChild(name)
        if t and t.Parent ~= ch then hum:EquipTool(t) end
    end

    _G.MonWebhook = _G.MonWebhook or function(url, content)
    ...
    -- Safe click (prevents nil Vector2 signature crash some exploits have)
    if not rawget(_G, "__MON_CLICK") then
        RunService.RenderStepped:Connect(function()
            if G.AutoFarm and (G.ClickAttack ~= false) then
                pcall(function()
                    VirtualUser:CaptureController()
    ...
    -- Safe quest GUI accessor to avoid infinite-yield on Main/Quest
    _G.MonGetQuestGui = _G.MonGetQuestGui or function()
        local pg = LP:FindFirstChild("PlayerGui")
        if not pg then return nil end
        local main = pg:FindFirstChild("Main") or (pg:FindFirstChild("Main") or pg:FindFirstChildOfClass("ScreenGui"))
        if not main then return nil end
        return main:FindFirstChild("Quest")
    end
end)

-- End MONARCH PATCH v1.1
